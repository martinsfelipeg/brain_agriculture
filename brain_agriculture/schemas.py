...
class FarmerSchema(BaseModel):
    ndoc: str = constr(pattern=r'^\d{11}|\d{14}$') # CPF: 11 digits, CNPJ: 14 digits
    name: str
    farm_name: str
    city: str
    state: str
    ...
    planted_crops: List[Literal['Soy', 'Corn', 'Cotton', 'Coffee', 'Sugar Cane']]
    ...
    def validate_areas(cls, value):
        ...
        if agric + veget > total:
            raise ValueError('The sum of agricultural and vegetation areas cannot exceed the total area of the farm.')
        return value
